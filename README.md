...
<!-- BEGIN COMPONENTS: generated by tools/readmegen from builder-config.yaml. DO NOT EDIT. -->
...
If you want to add a new component of otel collector, see the [Component Policy](docs/COMPONENT_POLICY.md) for the criteria, then modify [builder-config.yaml](builder-config.yaml) with `type` and `description` of the component. Then run `make build-src && make` to build the collector. `make build-src` also regenerates the component tables above from `builder-config.yaml`.
