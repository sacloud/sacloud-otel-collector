...
1. Edit `builder-config.yaml` and add the component's gomod reference with `type` and `description` (used to generate the README component table; ocb ignores them). Set `doc` to override the documentation link
2. Run `make build-src` to regenerate the source
3. Run `make sacloud-otel-collector` to build the binary
4. The README.md component table is regenerated by `make build-src` (or `make readme`); do not edit it by hand
5. Commit changes with descriptive message

## Component Management

- All available components are listed in `builder-config.yaml`
- The README.md component table is generated from `builder-config.yaml` by `tools/readmegen`, with documentation links pinned to the bundled component versions. CI runs `make check-readme` to detect a stale table
- Do not use `name` for this metadata: ocb uses it as the Go import alias
