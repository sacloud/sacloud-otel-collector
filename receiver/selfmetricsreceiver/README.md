...
- The synthetic metrics generated by the Prometheus scrape loop (`up` and
  `scrape_*` such as `scrape_duration_seconds`) are automatically dropped:
  `up` is always 1 when the collector scrapes itself, and the `scrape_*`
  statistics only describe the self-scrape.
